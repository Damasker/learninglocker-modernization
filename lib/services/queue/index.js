...
const getProvider = ({ queueProvider = process.env.QUEUE_PROVIDER }, done) => {
  try {
    switch (queueProvider) {
      case 'PUBSUB': return done(null, require('./pubsub'));
      case 'SQS': return done(null, require('./sqs'));
      case 'SERVICE_BUS': return done(null, require('./serviceBus'));
      case 'REDIS':
      case 'BULL':
        return done(null, require('./bull'));
      case 'LOCAL': return done(null, require('./local'));
      default: return done(new Error(`${queueProvider} is not a valid queue provider`));
    }
  } catch (err) {
    return done(new Error(`Failed to load ${queueProvider} queue provider: ${err.message}`));
  }
};
