...
describe('worker queue/notify contracts', () => {
  ...
  it('freezes post-ingest fan-out queue set with empty preReqs', () => {
    assert.deepStrictEqual(Object.keys(STATEMENT_POST_INGEST_QUEUES).sort(), [
      STATEMENT_EXTRACT_PERSONAS_QUEUE,
      STATEMENT_FORWARDING_QUEUE,
      STATEMENT_QUERYBUILDERCACHE_QUEUE,
    ].sort());
    Object.values(STATEMENT_POST_INGEST_QUEUES).forEach((queue) => {
      assert.deepStrictEqual(queue.preReqs, []);
    });
  });
});
