...
import { createStatementsRepo } from './xapiStatements/repo';
import getAttachments, {
  streamStatementsWithAttachments,
  boundary,
} from './xapiStatements/attachments';
import {
  getActivitiesFromStatement,
  getAgentsFromStatement,
  getRegistrationsFromStatement,
  getVerbsFromStatement,
} from './xapiStatements/queriables';
import { createSha } from './xapiStatements/testUtils';
...
describe('lib/kernel facades', () => {
  ...
  it('re-exports xapi-statements helpers', () => {
    assert.equal(typeof createStatementsRepo, 'function');
    assert.equal(typeof getAttachments, 'function');
    assert.equal(typeof streamStatementsWithAttachments, 'function');
    assert.equal(typeof boundary, 'string');
    assert.equal(typeof getActivitiesFromStatement, 'function');
    assert.equal(typeof getAgentsFromStatement, 'function');
    assert.equal(typeof getRegistrationsFromStatement, 'function');
    assert.equal(typeof getVerbsFromStatement, 'function');
    assert.equal(typeof createSha, 'function');
  });
});
