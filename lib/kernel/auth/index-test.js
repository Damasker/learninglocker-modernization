...
describe('auth kernel exports', () => {
  ...
  it('rejects unknown model names with Invalid scope', async () => {
    try {
      await getScopeFilter({
        modelName: 'notamodel',
        actionName: 'view',
        authInfo: { token: { tokenType: 'client' } },
      });
      assert.fail('expected Invalid scope error');
    } catch (err) {
      assert.equal(err.message, 'Invalid scope notamodel requested');
    }
  });
});
