...
# Dual-run statement analytics: legacy flag-off vs modern flag-on.
...
HOST_LABEL='${label}' bash lab/scripts/native-statement-aggregate-smoke.sh
EOF
}
...
mkdir -p lab/reports
scp -o BatchMode=yes \
  "${LEGACY_HOST}:/opt/learninglocker/app/lab/reports/ll-legacy-native-statement-aggregate.json" \
  lab/reports/
scp -o BatchMode=yes \
  "${MODERN_HOST}:/opt/learninglocker/app/lab/reports/ll-modern-native-statement-aggregate.json" \
  lab/reports/

node <<'NODE'
const fs = require('fs');
const legacy = JSON.parse(fs.readFileSync('lab/reports/ll-legacy-native-statement-aggregate.json', 'utf8'));
const modern = JSON.parse(fs.readFileSync('lab/reports/ll-modern-native-statement-aggregate.json', 'utf8'));
const names = ['aggregate', 'aggregateAsync', 'count', 'v1aggregate'];
const mismatches = [];
...
if (mismatches.length || !legacy.ok || !modern.ok) {
  console.error('NATIVE_STATEMENT_AGGREGATE_COMPARE_FAIL', { legacy, modern, mismatches });
  process.exit(1);
}
console.log('NATIVE_STATEMENT_AGGREGATE_COMPARE_OK', {
